...
export const getUserInfo = (token) => {
  ...
  if (user) {
    ...
    const highestLevel = Math.max(
      ...userLevels.map((l) => parseInt(l.split("_")[1]))
    );
    const allFeatures = [
      ...new Set(
        user.accounts.flatMap((a) => ACCOUNT_LEVELS[a.level].features)
      ),
    ];
...
export const transfer = (token, fromAccount, toAccount, amount, remark) => {
  ...
  const fromUser = users.find((u) =>
    u.accounts.some((a) => a.accountNumber === fromAccount)
  );
  const toUser = users.find((u) =>
    u.accounts.some((a) => a.accountNumber === toAccount)
  );
  ...
  const fromAccountInfo = fromUser.accounts.find(
  ...
  if (amountNum > accountLevel.dailyTransferLimit) {
    return mockResponse(
      null,
      false,
      `超过账户等级${accountLevel.name}的每日转账限额`
    );
  }
  ...
  const toAccountInfo = toUser.accounts.find(
    (a) => a.accountNumber === toAccount
  );
  toAccountInfo.balance = (
    parseFloat(toAccountInfo.balance) + amountNum
  ).toFixed(2);
...
export const getTransferRecords = (accountNumber, page = 1, pageSize = 10) => {
  const records = transferRecords
    .filter(
      (r) => r.fromAccount === accountNumber || r.toAccount === accountNumber
    )
    .sort((a, b) => new Date(b.createTime) - new Date(a.createTime));
...
// 模拟人脸识别登录接口
export const faceLogin = (faceImage, userId) => {
  console.log("人脸识别登录");
  // 模拟人脸识别成功
  const mockUser = {
...
export const uploadImage = (file) => {
  return mockResponse({
    url: "https://10.100.86.246:8083/api/upload" + file.name,
    filename: file.name,
  });
};
